...
st.write("""
    ...
    В качестве движка используется бесплатный переводчик DeepL. \n
    Как только лимит перевода достигнут, переводчик прекращает работу.\n
    При помощи данного генератора лучше переводить слова по-отдельности во избежание замены частей речи, поскольку, например, в длинных предложениях глагол языка А может переводиться существительным на язык Б
""")
